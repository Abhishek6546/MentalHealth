...
const BreathingExercise = () => {
  ...
  return (
    <div
      className={`rounded-3xl shadow-2xl p-8 transition-all duration-300 border 
        ${mode === "dark" 
    ...
    >
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold mb-2">Guided Breathing</h2>
        <p className={`mb-8 ${mode === "dark" ? "text-gray-400" : "text-gray-600"}`}>
        ...
        <div className="relative w-64 h-64 mx-auto mb-8">
          ...
          <div className={`rounded-full mx-auto flex items-center justify-center font-semibold text-lg shadow-2xl transition-all ease-in-out duration-[4000ms] ${
            ...
            <div className="text-center">
              <div className="text-xl font-bold">{currentPhase.label}</div>
              <div className="text-sm opacity-90 mt-1">{currentPhase.instruction}</div>
            </div>
          </div>
          ...
          <div className="absolute inset-0 rounded-full border-4 border-gray-300 dark:border-gray-600">
            <div
              className="absolute inset-0 rounded-full border-4 border-blue-500 border-t-transparent transition-all duration-100"
        ...
        <div className="flex justify-center items-center gap-6 mb-8">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-300">{timeLeft}s</div>
            <div className={`text-sm ${mode === "dark" ? "text-gray-400" : "text-gray-500"}`}>Time Remaining</div>
          </div>
          <div className="w-px h-8 bg-gray-300 dark:bg-gray-600" />
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600 dark:text-purple-300">{Math.floor(elapsed / 1000)}s</div>
            <div className={`text-sm ${mode === "dark" ? "text-gray-400" : "text-gray-500"}`}>Completed</div>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-4">
          <button
            ...
            className={`flex items-center gap-2 px-8 py-3 rounded-full font-semibold transition-all duration-300 shadow-lg ${
              isActive 
        ...
        {/* Completion Message */}
        {!isActive && elapsed > 0 && (
          <div className="mt-6 p-4 rounded-xl bg-gradient-to-r from-green-100 to-emerald-100 dark:from-green-900 dark:to-emerald-800">
            <p className="font-semibold text-lg text-green-700 dark:text-green-300">Session Complete! 🌟</p>
            <p className="text-sm text-green-600 dark:text-green-400">Well done on completing your breathing exercise</p>
          </div>
        )}
      </div>
    </div>
